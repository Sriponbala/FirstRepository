...
public class MultipleInheritance
{
    ...
    //UPCASTING IN INTERFACES    
        Father father = new Child(child_name, year);
        father.fatherDetails(child_name);
        father.playingCricket();
        father.singing();
        System.out.println(father.father_name);
        System.out.println();
        
   //DOWNCASTING FATHER TO CHILD
        Child childDowncast = (Child)father;
        childDowncast.fatherDetails(child_name);//Father,s default method
        childDowncast.motherDetails(child_name);// Mother's default method
        childDowncast.playingChess();//from child class and mother interface
        childDowncast.playingCricket();//child  - father
        childDowncast.playsFootball();//child 
        childDowncast.singing();//child  - mother - father
	}
} 
